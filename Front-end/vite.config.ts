...
export default defineConfig(async ({ mode }) => {
  ...
  try {
    const siteMod = await import('../.figma/make/site.json', { with: { type: 'json' } })
    siteConfiguration = siteMod.default || {}
  } catch {
    siteConfiguration = {}
  }
...
/** Serves index.html for all non-asset paths so the React SPA can handle /admin routing. */
function spaFallback(): Plugin {
  return {
    name: 'figma-spa-fallback',
    apply: 'serve',
    configureServer(server) {
